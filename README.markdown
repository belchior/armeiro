...
// possível acessar os mapeamentos definidos no arquivo package.json
...
```
## Modulos
Para tornar um modulo disponível é necessário incluir-lo como uma dependência no seu arquivo package.json ou instala-lo manualmente usando a opção `--save-dev`.
...
```
## Lista de Comandos
Todos os comando usam o objeto `armeiro.[module].orig` para carregar arquivos e salva os arquivos gerados em um diretório definido em `armeiro.[module].dest` . Consulte os [Modulos](https://github.com/belchior/armeiro#modulos) disponível para mais informações.
#### compile
